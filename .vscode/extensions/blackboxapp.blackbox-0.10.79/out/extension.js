...
function activate(_) {
	...
	vscode.commands.registerCommand("blackbox.codeSearchShortcut", () => {
		triggerCodeSearchFromInputBox()
		selectionFct('code search from palette')
	})
	vscode.commands.registerCommand("extension.keyBindingOpenCodeSearchShortcut", () => {
		triggerCodeSearchFromInputBox()
		selectionFct('code search from keyboard')
	})
	async function triggerCodeSearchFromInputBox(){
		let query = await vscode.window.showInputBox({
			placeHolder: "Blackbox Code Search (CMD+SHFT+A or CTRL+ALT+A)",
			prompt: "Type your question here",
			ignoreFocusOut: true
		})
		if (query){
			let rs = await (0, search_1.search)(
				query,
				userId
			)
			if (rs) {
				let items = rs.results.map((item) => {
					acceptType = "Search"
					codeReturn = item.code
					vscode.commands.executeCommand(
						"blackbox.showCode"
					)
				})
		...
		`
			...
			<body>
				...
				<l class='text-description'>
					...
					4. Open Quick Start Guide<br><br>
					...
					while(iterations < max_iterations && completed == false){
						try{
							...
							if (result['completed'] == true || result['response'][0][0] == obj['stream']){
								completed = true
								postMessage({command:"removeTyping", id})
							}
		...
		return `
			...
			<body>
				<div class="chat-connected active">
					...
					<div class="message-disclaimer">Blackbox AI Chat is in beta and Blackbox is not liable for the content generated. Blackbox may produce inaccurate information about people, places, facts, code and more.</div>
				</div>
				<script src="https://code.jquery.com/jquery-3.6.3.js" integrity="sha256-nQLuAZGRRcILA+6dMBOvcRh5Pe310sBpanc6+QBmyVM=" crossorigin="anonymous"></script>
				<script>
				(function() {
						...
						if(answerArr.length > 1){
							answerArr.forEach((answerObj, idx)=>{
								...
								if(type === "code"){
									...
									$(chatMessageText).append("<span>"+str+"</span>")
								}
							})
						}
						else{
					...
					function showFeedback($holder){
						const $el = $('<div class="options-holder"><img src="${thumbsDownSrc}" class="bad-icon option-icon"><img src="${thumbsUpSrc}" class="good-icon option-icon"></div>')
						...
						$el.find(".option-icon").on("click", function(){
							// 1 means thumbs up
							var type = 1
							if($(this).hasClass("bad-icon")){
								// -1 is thumbs down
							...
							postMessage({command: "answerFeedback", query: lastSearchedQuery, answer: lastAnswer, type: type})
					...
					window.addEventListener('message', event => {
						...
						if(data.command === "showAnswer"){
							...
							$(".chatMessage[data-id="+data.id+"]").find(".typing").remove()
						}
					});
				}())
				</script>
